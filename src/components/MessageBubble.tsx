...
function barHeight(index: number): string {
  const wave =
    Math.abs(Math.sin(index * 1.7)) * 0.7 +
    Math.abs(Math.sin(index * 0.6)) * 0.3;
  ...
  return (
    <figure
      className={`message-bubble${
        message.channel ? " message-bubble--channel" : ""
      }`}
    >
      ...
      <div className="message-bubble__main">
        {message.channel ? (
          ...
          <figcaption className="message-bubble__meta">
            ...
            {message.meta ? (
              <span className="message-bubble__meta-info">
                {message.meta}
              </span>
            ) : null}
          </figcaption>
        ...
        {!message.audio && message.voice ? (
          ...
          >
            <span
              className="message-bubble__voice-icon"
              aria-hidden="true"
            >
            ...
            <span
              className="message-bubble__wave"
              aria-hidden="true"
            >
        ...
        {transcript ? (
          <p
            className="message-bubble__transcript"
            lang={message.audio?.language}
          >
            {transcript}
          </p>
        ) : null}
